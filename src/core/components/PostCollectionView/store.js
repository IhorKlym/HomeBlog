...
class PostCollectionViewStore {
  ...
  fetchPosts = flow(function* fetch(id: any, fetchMore: ?boolean) {
    ...
    try {
      ...
      const resp = yield systemTagsApi.getSystemTag(id, params);
      const { meta, data: { posts } } = resp;
      let data = posts;
      data = data.map(post => ({
        ...post,
...
export default new PostCollectionViewStore();

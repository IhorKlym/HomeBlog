...
const PostCollectionView = ({ data, handleClose, ...props }: Props) => {
  const windowSize = useWindowSize();
  const [columnWidth, setColumnWidth] = useState('33.33%');
  // const [showFilters, setShowFilters] = useState(false);
  const gridRef = useRef(null);
  const { id, name, postSystemTagsCount } = data;
  const { page, loading, totalFetchedCount, posts, itemsCountOnPage } = store;
  // const { dictionariesStore } = stores;
    ...
    <S.Wrap {...props}>
      <S.Container>
        ...
        {/* <S.InfoRow>
          ...
          <S.FiltersToggle onClick={() => setShowFilters(!showFilters)}>
            Filters {showFilters ? '-' : '+'}
          </S.FiltersToggle>
        </S.InfoRow> */}
        ...
        <S.PostsList>
          <InfiniteScroll
            ...
            hasMore={!loading && postSystemTagsCount > posts.length}
          >
            {!!posts.length &&
              <StackGrid
...
export default observer(PostCollectionView);
